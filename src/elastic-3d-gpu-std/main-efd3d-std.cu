...
int main(int argc, char* argv[]) {
  ...
  /*------------------------------------------------------------*/
    ...
    if(! sf_getbool("interp",&interp)) interp=false; /* perform linear interpolation on receiver data */
...
#pragma omp parallel for
  ...
  sf_warning("\ntotal iter: %d, time elapsed for computation  (not including output wavefield/data): %.4fs", it, compute_elapsed);
